...
DarkEdif data blocks and lifecycle (Windows)
--------------------------------------------
* Editdata (design-time): Stored in Fusion MFAs and allocated by DarkEdif using the JSON-defined properties; default layout is auto-managed so long as you keep JSON properties in sync. If you enable `NOPROPS`, you must handle property persistence manually in the edittime getters/setters. The `extVersion` you set in `CreateObject` controls when `UpdateEditStructure()` runs to migrate existing objects. 【F:DarkEdif/#MFAs and documentation/Edif to DarkEdif conversion.md†L4-L24】
* Rundata (runtime): DarkEdif builds the runtime structure for you and forwards calls into `Extension`. Flags such as `OEFLAGS::SPRITES`, `OEFLAGS::TEXT`, and related prefs shape what the runtime allocates (e.g., sprite/animation bookkeeping). Avoid function-scope statics that rely on thread-safe initialization on XP; prefer member or file-scope state. 【F:DarkEdif/Inc/Shared/AllPlatformDefines.hpp†L87-L193】【F:DarkEdif/#MFAs and documentation/DarkEdif MultiTargeting.md†L118-L154】
* Lifecycle checkpoints:
  * Create/edit time: JSON-driven properties fill `EDITDATA`; `UpdateEditStructure()` migrates when `extVersion` changes. 【F:DarkEdif/#MFAs and documentation/Edif to DarkEdif conversion.md†L4-L24】
  * Runtime construction: DarkEdif sets up `RUNDATA`/`Extension`, links ACE tables from `DarkExt.json`, and validates parameter/return types in debug builds. 【F:README.md†L104-L114】
  * Runtime use: ACE calls use the jump table generated by the prebuild tool; properties accessed through `DarkEdif::Properties` stay consistent with JSON smart props. 【F:DarkEdif/DarkEdif Template/DarkExt.json†L2-L136】
  * Shutdown: DarkEdif tears down runtime state; ensure you release external resources and stop threads in your `Extension` destructor or equivalent cleanup hooks.
